...
def main():
    ...
    for i in range(1,len(brr_data)):
        ...
        y_data = min_h2_kappa[Y_COL].to_numpy(float)[0:i]
        ...
        cf = ax1.contourf( Ag_deg, Rg, Zm, levels=clevels, cmap=cmap,linewidths=10, norm=norm, antialiased=True, extend = "min")
        ...
        ax1.scatter([70.3], [1], c=[0.76], cmap=cmap, norm=norm, edgecolors="green", linewidths=1.4, s=70, label="data")
        ax1.scatter([50], [1.71], c=[0.65], cmap=cmap, norm=norm, edgecolors="cyan", linewidths=0.6, s=70, label="data")
        ...
        cbar.ax.set_xlabel(r'$E_{min}$', labelpad=5)
