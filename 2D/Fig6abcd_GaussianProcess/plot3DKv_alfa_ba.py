...
OUT_2D_PNG = r"C:\Users\User\File\run4Kv.png"
VORM_COL = "vorm"
A_COL    = "a(µm)"
B_COL    = "b(µm)"
Y_COL    = "kappa(m^2)"
...
def invalid_mask(alpha_deg_grid, ratio_grid, e=0.5, W=2.0):
    ...
    hoek = 0.5 * alfa_rad
    hoek = np.clip(hoek, 1e-9, np.pi/2 - 1e-9)
    L = W / np.tan(hoek)
...
def main():
    ...
    y_data = min_h2_kappa[Y_COL].to_numpy(float)
    ...
    ax1.scatter([70.3], [1], c=[6.43e-14], cmap=cmap, norm=norm, edgecolors="green", linewidths=1.4, s=70, label="data")
    ax1.scatter([50], [1.71], c=[1.16e-13], cmap=cmap, norm=norm, edgecolors="cyan", linewidths=0.6, s=70, label="data")
    ...
    cbar.ax.set_xlabel(r'$K_{v} \mathrm{(m^2)}$', labelpad=12)
